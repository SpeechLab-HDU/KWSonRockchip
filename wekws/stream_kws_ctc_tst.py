...
sys.path.insert(0, "../")
sys.path.append("/home/liuhaoqi/PycharmProjects/wekws-main")
sys.path.append("/home/liuhaoqi/PycharmProjects/wekws-main/wekws")
...
class KeyWordSpotter(torch.nn.Module):
    ...
    def execute_detection(self, t):
        ...
        duration = end - start
        if hit_keyword is not None:
            self.last_hit_score = self.hit_score
...
def find_index(key_words, hit_word):
    for index, (word, _) in enumerate(key_words.items()):
        if word == hit_word:
            return index
    return None


def stream_ctc(kws_model, stream_data):
    global hit_index_global
    # 对输入的数据进行处理
    # wav = (stream_data * (1 << 15)).astype("int16").tobytes()
    wav = (stream_data).astype("int16").tobytes()
    ...
    for i in range(0, len(wav), interval):
        ...
        result = kws_model.forward(chunk_wav)
        if 'state' in result and result['state'] == 1:
            hit_keyword = result['keyword']
            with lock:
                hit_index_global = find_index(kws_model.keywords_token, hit_keyword)


def listen_hit_index():
    global hit_index_global
    while True:
        time.sleep(0.01)
        with lock:
            if hit_index_global == -1:
                continue
            stream.save_log(hit_index_global, kws.last_hit_score)
            # 检测到关键词后的事件可以更改
            print('监听到的关键词标签为:{}'.format(hit_index_global))
            if hit_index_global == 0:  # 我要上楼
                playsound.playsound("../Feedback/正在上楼.mp3")
            elif hit_index_global == 1:  # 我要下楼
                playsound.playsound("../Feedback/正在下楼.mp3")
            elif hit_index_global == 2:  # 我要去负一楼
                playsound.playsound("../Feedback/正在去负一楼.mp3")
            elif hit_index_global == 3:  # 我要去一楼
                playsound.playsound("../Feedback/正在去一楼.mp3")
            elif hit_index_global == 4:  # 我要去二楼
                playsound.playsound("../Feedback/正在去二楼.mp3")
            elif hit_index_global == 5:  # 我要去三楼
                playsound.playsound("../Feedback/正在去三楼.mp3")
            elif hit_index_global == 6:  # 我要去四楼
                playsound.playsound("../Feedback/正在去四楼.mp3")
            elif hit_index_global == 7:  # 我要去五楼
                playsound.playsound("../Feedback/正在去五楼.mp3")
            elif hit_index_global == 8:  # 电梯关门
                playsound.playsound("../Feedback/正在关门.mp3")
            elif hit_index_global == 9:  # 电梯开门
                playsound.playsound("../Feedback/正在开门.mp3")
            hit_index_global = -1  # 重置 hit_index_global
...
def record_callback(Data):
    stream_ctc(kws, np.frombuffer(Data, dtype=np.int16))


if __name__ == '__main__':
    os.chdir('/home/liuhaoqi/PycharmProjects/wekws-main/wekws')
    hit_index_global = -1
    ...
    # 创建监听线程
    listen_thread = threading.Thread(target=listen_hit_index)
    listen_thread.daemon = True  # 设置为守护线程，随着主线程的结束而结束
    listen_thread.start()

    # 开启音频接收
    stream = StreamAudio(record_callback, chunk=9600, rate=16000)

    # 退出前执行before_exit函数释放麦克风
